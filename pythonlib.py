...
def soft_maxGrad(x,y,theta,lamda,iterations,probM,xtest,ytest):
	for i in range(iterations):
		cost,subsum,alpha = softmaxCost(x,y,theta,lamda)
		prob = -(np.exp(x.dot(theta)-alpha)/subsum)
...
def softmaxCost(x,y,theta,lamda):
	...
	alpha = np.max(x.dot(theta))
	subsum = sum(np.nan_to_num(np.exp(x.dot(theta)-alpha)).transpose()).reshape(-1,1)
	#print(subsum.shape)
